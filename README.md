...
All of this section is regarding the configuration of [code/features_02/config_02.py](code/features_02/config_02.py) file.
...
- `WebDriverPath`
  ...
  For now, the default will be using Chrome webdriver, but you can change it to firefox (gecko driver) if you want. I will describe how to do it in the code, when webdriver is initialized on function init_webdriver at [lib_common.py](code/features_02/lib_common.py).

- `ShowBrowserWindows`
  - `True`: browser will be shown
  - `False`: browser will be run in headless mode (hidden)
  
  I recommend to run the browser as shownn first. After all the process is succeed, you can hide it for better performance (a hidden browser will consume less GPU and memory).


- `UseProxies`
  - `True`: will obtain proxy from [Proxies.txt](code/features_02/Proxies.txt)
  - `False`: browser will run without proxy enabled.
  
  Proxy configuration will always be done just before the webdriver is initialized (i.e. browser is opened). We can not change the proxy in the fly, so when change proxy process is about to commence, browser will be closed and recreated with the next specified proxy.
...
- `SequentialFiles`
  ...
  Mode options to download the files. Sequential (i.e. using requests module) or not (i.e. using selenium click).
  - `True`: The download will be done with requests module in the background. This will make sure the file is downloaded completely and wait for interval.
...
- `RequestHeatder` (dictionary)
  ...
  By default, I filled it with user-agent of Chrome browser, but you can change it to other user-agent such as firefox, safari, etc.
...
- `SaveDirectory` and `DefaultSaveDirectory`
  ...
  You can specify both absolute path and relative path. The default download directory will be at [download folder](code/features_02/download)
...
Most of them need around 30-60 secs of request just to solve the simplest captcha (reading number/text from picture), and it is not even at 100% accuracy.
As for this day, I still can not find a service to solve google's captcha.


Should you find a big problem here, it is either to solve manually (the code will wait for you to solve), or I would suggest this approach as the currently best options:
...
The step-by-step above is literally doing the login by human, and continue to automate with code with given cookies.
...
documentation of the API service. Feel free to inform me about this and I will make it)
...
- `AntiCaptchaAPIKey`
  ...
  This is API key for anti captcha services.
...
- `XPathLoginSuccessProof`
  ...
  The idea is to check wheter a certain element is *exist* after we click login button, with a timeout for several seconds.

  If the element *do* appears, then we can be sure that the login is succeed.

  If not, then the code may continue, but it will be suspectible of an error because the code can not guarantee that the session is obtained.
...
- `XPathFiles`
  ...
  XPath to be searched for downloads, later it will be clicked or opened
...
- `IntervalsBetweenFiles`
  ...
  - Give it an integer: it will wait for a fixed time
    ...
    `IntervalsBetweenFiles = 20` : fixed wait for 20 seconds

  - Give it a tuple: it will wait for a range of time

    `IntervalsBetweenFiles = (20, 30)` : randomly wait between 20 to 30 seconds
