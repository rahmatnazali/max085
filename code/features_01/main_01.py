...
If Done.csv is not given, code will return empty list (means no occurence happened yet)
...
If there is not (means no occurence happened yet), code will append it, and add to this dictionary to marked it as "occured"
        ...
        for every cofig.XPath attribute we have:
            ...
            if found:
                ...
                for every row:
                    ...
                    if not there (means not occured) --> insert to data csv --> mark this occurence in occured_data_dict
...
for url in enumerate(url_list):
    ...
    if config.TestMode:
        # for testing purpose
        with open("example_2.html") as html_file:
    ...
    if data_found_length:
        ...
        for i in range(data_found_length):
            ...
            If href is not in link_list, add it to the list, and update the occured data, so that no occurence of this element will appear.
            """
